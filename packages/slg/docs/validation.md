...
## 2026-09-17 真实存储下的游戏强杀恢复

新增独立slg测试组，两轮各7/7通过，共16次游戏进程强杀。最终经统一入口运行，报告 `temp/recovery/run-iaAz8o/report.json`；包含建筑、抽卡、武将、行军/占领、抢地和离线产粮的SQL对账。细节与启动夹具失败教训见[恢复测试](recovery-test.md)。这补充下方历史记录中“未执行真实强杀”的范围；不代表DBProxy自身故障、500玩家、热更或长稳通过。

## 2026-09-17 基础玩法

建筑升级、整分钟粮食、抽卡/武将升级、行军占领已接入。15 条单测、完整 build、隔离内存模式真实 RPC smoke 通过；测试步骤、首次失败原因与未验证范围见 [基础玩法验收](gameplay-demo.md#测试步骤和当前证据)。未执行新增 UI 画面预览、真实玩法数据库强杀恢复或三类联合演练。以下为历史记录，不代表新玩法已通过这些历史验收。
