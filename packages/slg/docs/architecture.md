...
当前只有一个 `SlgWorldScene`，装饰器默认去掉 Scene 后缀，部署配置使用 `sceneType: SlgWorld`。它提供公开只读快照，不承接玩家经济状态。Handler 只调用领域方法；Model 定义类型身份，Hotfix 提供快照行为。
...
开发环境已配置专属 DBProxy/PostgreSQL/Redis，入口为本机 18700，凭据只从 Git 忽略的本地文件注入游戏服务端。玩家/行军可变状态接入仍待实现。持久化不能用全局变量或本地 JSON 冒充，不复用其他游戏的 namespace、账号和数据库配置。

## 开发体验待办

优先用真实业务需求检验：新协议是否一次生成完成、错误是否直接定位模块、普通业务改动是否不需重复构建 Rust、重启是否一条命令。当前统一入口已完成；文件监听/自动 Hotfix、模块 Luban 模板与玩家持久化闭环仍待接入。
