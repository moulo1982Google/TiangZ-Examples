...
const runtime = startRuntime(
  root,
  "configs/local/gate-failover/StartMachine.json",
  "gate-failover",
  "debug",
);
let probe;
...
try {
  await Promise.all([7001, 7201, 7202, 7302, 7401].map((port) => waitForPort(port, runtime, 30_000)));
  await sleep(5_500);
  probe = startProbe();
  const ready = await waitForMarker(probe, "GATE_FAILOVER_READY", 30_000);
  const configName = ready.gateName === "gate_1" ? "gate-1.json" : "gate-2.json";
  const failedPort = Number(ready.gatePort);
  const failedPid = await findDirectChildByConfig(runtime.child.pid, configName);
  process.kill(failedPid, "SIGKILL");
  await waitForPortClosed(failedPort, 3_000);
  probe.child.stdin.write("continue\n");

  const taken = await waitForMarker(probe, "GATE_FAILOVER_TAKEN", 30_000);
  const restartedPid = await waitForDirectChildReplacement(
    runtime.child.pid,
    configName,
    failedPid,
    30_000,
  );
  await waitForPort(failedPort, runtime, 30_000);
  probe.child.stdin.write("restarted\n");
  const passed = await waitForMarker(probe, "GATE_FAILOVER_PASSED", 30_000);
  const exit = await waitForExit(probe, 10_000);
  if (exit !== 0) throw new Error(`Gate failover probe exited ${exit}:\n${probe.output()}`);
  console.log("[gate-failover] passed", {
    failedPid,
    restartedPid,
    previousGate: taken.previousGate,
    currentGate: passed.currentGate,
    unitId: passed.unitId,
  });
} catch (error) {
  const logs = writeFailureLogs(root, "gate-failover", [runtime]);
  console.error(`[gate-failover] failure logs: ${logs}`);
  throw error;
} finally {
...
function startProbe() {
  const child = execFile(process.execPath, [path.join(root, "dist", "gate_failover_probe.cjs")], {
    cwd: root,
    windowsHide: true,
    maxBuffer: 1024 * 1024,
  });
...
function waitForMarker(target, marker, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    const inspect = () => {
      ...
      if (target.child.exitCode !== null) return reject(new Error(`probe exited before ${marker}:\n${target.output()}`));
      if (Date.now() >= deadline) return reject(new Error(`timed out waiting for ${marker}:\n${target.output()}`));
