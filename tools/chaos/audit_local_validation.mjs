...
assertCoverage(started.actions, recovered.map(e => e.action));
assert.ok(recovered.every(e => e.unchangedAccounts === true && e.passingRounds >= 2), "business recovery must prove two rounds with unchanged accounts");
...
assert.equal(soakFinal?.checks?.finalClientVisibleStatePassed, true, 'rebuild soak: explicit final-state result required');
assert.equal(soakFinal?.checks?.observedConsistencyPassed, true, 'historical old reads cannot be hidden by recovery');
